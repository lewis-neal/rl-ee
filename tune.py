...
for env_name in env_names:
    ...
    for val in c_vals:
        ...
        for seed in seeds:
            print('Seed = ' + str(seed))
            env.seed(seed)
            action_selector = UCB_1(val, env.get_total_states(), env.get_total_actions(), discount_factor)#EpsilonGreedy(epsilon, val, seed)#MBIE_EB(val, env.get_total_states(), env.get_total_actions(), discount_factor)#Boltzmann(val, seed)
            q_function.reset()
            logger = Logger(episodes)
            filepath = log_dir + '/UCB-1-C-' + str(val) + '-' + str(seed)
